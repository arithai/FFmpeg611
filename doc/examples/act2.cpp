...
//printf("%s(%d)\n",__FILE__,__LINE__);  
  ptList=ptHead;
  while(ptList!=NULL && ptList->pt.w>=weight) { //>=şŮĄý¨ěĄýÄą   8,7,7,5
//  printf("%s(%d)\n",__FILE__,__LINE__);  
    if(ptList->next!=NULL) {
...
//printf("%s(%d)\n",__FILE__,__LINE__);  
  ptListG=ptHeadG;
  while(ptListG!=NULL && ptListG->pt.w>=weight) { //>=şŮĄý¨ěĄýÄą   8,7,7,5
//  printf("%s(%d)\n",__FILE__,__LINE__);  
...
//    if(Tw>70) exit(0);
  ...
  printf("FindXYZSTLineList (%d,%d) end\n",width,height);
//if(Tfi==7) exit(0);
...
//}
  ...
  edgeList=edgeHead;
  while(edgeList!=NULL && edgeList->x<x) { //Ąý¨ěĄhĄ­§ˇ
...
void DrawEdge(int width,int height,int index) {
  ...
  int x0,y0,x1,y1,x3,y3;
  int pos,pos1,pos2;
  int i,sum,avg;
  int *ylist;
  pos=0;pos1=0;pos2=0;pos=pos;pos1=pos1;pos2=pos2;
  x0=0;x1=0;
  while(edgeList!=NULL) {
...
//int g,R1,G1,B1,R0,G0,B0;
  ...
  //    g=abs(a(G0)-a(G1));
  //    #if 1
  //    if(ge> 8 && abs(ax(G1)-ax(G0)) > 8 ) { //ĹÜ°ĘĄB¬OĂä¬É
  //    #else
  //    if(marginalV(x2,y2)) {
  //    #endif
...
// Set the locale to the user's default environment
  ...
  *(temp + 3) = 0;
  ...
  wcscat(pDest, fixstr);
  swprintf(pDest, size, L"%wsV%02d%02d%02d.01\n",fixstr, year, month, day);
}
...
std::string WStringToString(const std::wstring& wstr) {
    ...
    WideCharToMultiByte(CP_UTF8, 0, &wstr[0], (int)wstr.size(), &buffer[0], size_needed, NULL, NULL);
    
    return std::string(&buffer[0], size_needed);
}
...
void mProduct(double x,double y) {
  ...
  vector_t *xv=(vector_t *)ivector6_new(1,0,1,0,1,2);
  matrix_t *A=(matrix_t *)matrix_from_vectors(u1,u2);
  matrix_t *b=(matrix_t *)SolveB_from_Ax(A,xv);
  matrix_print(b);
#endif
}
